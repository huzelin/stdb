...
#include "stdb/query/query_processing/limiter.h"
...
common::Status SeriesRetreiver::add_tag(std::string name, std::string value) {
  if (metric_.empty()) {
    LOG(ERROR) << "Metric not set";
    return common::Status::BadArg();
  }
  if (!series_.empty()) {
    LOG(ERROR) << "Series already set";
    return common::Status::BadArg();
  }
  if (tags_.count(name)) {
    // Duplicates not allowed
    LOG(ERROR) << "Duplicate tag '" + name + "' found";
    return common::Status::BadArg();
  }
  tags_[name] = { value };
...
common::Status SeriesRetreiver::add_tags(std::string name, std::vector<std::string> values) {
  if (metric_.empty()) {
    LOG(ERROR) << "Metric not set";
    return common::Status::BadArg();
  }
...
common::Status SeriesRetreiver::add_series_name(std::string name) {
  if (!tags_.empty()) {
    LOG(ERROR) << "Tags already set";
    return common::Status::BadArg();
  }
...
std::tuple<common::Status, std::vector<ParamId>> SeriesRetreiver::extract_ids(SeriesMatcher const& matcher) const {
  ...
  if (!series_.empty()) {
    ...
    if (ids.empty()) {
      return std::make_tuple(common::Status::NotFound(), ids);
    }
    ...
    for (i64 id: sids) {
      ids.push_back(static_cast<ParamId>(id));
    }
  } else {
    ...
    for (auto tup: search_results) {
      ids.push_back(static_cast<ParamId>(std::get<2>(tup)));
    }
    if (metric_.size() > 1) {
      std::vector<std::string> tail(metric_.begin() + 1, metric_.end());
      std::vector<ParamId> full(ids);
      for (auto metric: tail) {
        for (auto id: ids) {
          ...
          if (name.second == 0) {
            ...
            LOG(FATAL) << "Matcher data is broken, can read series name for " + std::to_string(id);
          }
          ...
          full.push_back(static_cast<ParamId>(sid));
  ...
  return std::make_tuple(common::Status::Ok(), ids);
}

std::tuple<common::Status, std::vector<ParamId>> SeriesRetreiver::extract_ids(PlainSeriesMatcher const& matcher) const {
  std::vector<ParamId> ids;
  // Three cases, no metric (get all ids), only metric is set and both metric and tags are set.
  if (metric_.empty()) {
    ...
    if (tags_.empty()) {
      ...
      for (auto res: results) {
        ids.push_back(static_cast<ParamId>(std::get<2>(res)));
      }
    } else {
      ...
      for (auto metric: tail) {
        for (auto id: ids) {
          ...
          if (name.second == 0) {
            // This shouldn't happen but it can happen after memory corruption or data-race.
            // Clearly indicates an error.
            LOG(FATAL) << "Matcher data is broken, can read series name for " << id;
          }
          std::string series_tags(name.first + first_metric.size(), name.first + name.second);
          std::string alt_name = metric + series_tags;
          auto sid = matcher.match(alt_name.data(), alt_name.data() + alt_name.size());
...
std::tuple<common::Status, std::vector<ParamId>> SeriesRetreiver::fuzzy_match(PlainSeriesMatcher const& matcher) const {
  std::vector<ParamId> ids;
  // Three cases, no metric (get all ids), only metric is set and both metric and tags are set.
  if (metric_.empty()) {
    // Case 1, metric not set.
    return std::make_tuple(common::Status::BadArg(), ids);
  } else {
    auto first_metric = metric_.front();
    if (tags_.empty()) {
      // Case 2, only metric is set
...
static std::tuple<common::Status, std::string, ErrorMsg> parse_search_select_stmt(boost::property_tree::ptree const& ptree) {
  ...
  return std::make_tuple(common::Status::QueryParsingError(), "", "Query object doesn't have a 'select' field");
}

static std::tuple<common::Status, std::string, ErrorMsg> parse_select_stmt(boost::property_tree::ptree const& ptree) {
  auto select = ptree.get_child_optional("select");
  if (select && select->empty()) {
    // select query
    auto str = select->get_value<std::string>("");
...
static std::tuple<common::Status, std::string, ErrorMsg> parse_select_events_stmt(boost::property_tree::ptree const& ptree) {
  ...
  if (select && select->empty()) {
    ...
    if (!str.empty() && str.front() == '!') {
      ...
      return std::make_tuple(common::Status::QueryParsingError(), "", "Event name should start with '!' symbol");
    }
  }
  return std::make_tuple(common::Status::QueryParsingError(), "", "Query object doesn't have a 'select-events' field");
}

static std::tuple<common::Status, std::string, ErrorMsg> parse_select_events_filter_field(boost::property_tree::ptree const& ptree) {
  ...
  if (flt && flt->empty()) {
    ...
    if (!str.empty()) {
      try {
        ...
        return std::make_tuple(common::Status::BadArg(), "", w.what());
      }
      return std::make_tuple(common::Status::Ok(), str, ErrorMsg());
    }
  }
  return std::make_tuple(common::Status::Ok(), "", "");
}

/** Parse `join` statement, format:
 * { "join": [ "metric1", "metric2", ... ], ... }
 */
static std::tuple<common::Status, std::vector<std::string>, ErrorMsg>
    ...
    for (auto item: *join) {
      ...
      if (value) {
        ...
        if (!str.empty() && str.front() != '!') {
          ...
          return std::make_tuple(common::Status::QueryParsingError(), result, "Metric name can't be empty or start with '!' symbol");
        }
      } else {
        return std::make_tuple(common::Status::QueryParsingError(), result, "Metric name expected");
      }
    }
  }
  if (result.empty()) {
    return std::make_tuple(common::Status::QueryParsingError(), result, "Metric name should be set");
  }
  return std::make_tuple(common::Status::Ok(), result, ErrorMsg());
}

/** Parse `aggregate` statement, format:
 * { "aggregate": { "metric": "func" }, ... }
 */
static std::tuple<common::Status, std::vector<std::string>, std::vector<std::string>, ErrorMsg> parse_aggregate_stmt(boost::property_tree::ptree const& ptree) {
  ...
  if (aggregate) {
    ...
    for (auto kv: *aggregate) {
      ...
      if (!metric_name.empty() && metric_name.front() != '!') {
        ...
        return std::make_tuple(common::Status::QueryParsingError(), EMPTY, EMPTY, "Metric name can't be empty or start with '!' symbol");
      }
      functions.push_back(func);
    }
    if (metrics.empty()) {
      return std::make_tuple(common::Status::QueryParsingError(), EMPTY, EMPTY, "Query object has empty `aggregate` field");
    }
    return std::make_tuple(common::Status::Ok(), metrics, functions, ErrorMsg());
  } else {
    return std::make_tuple(common::Status::QueryParsingError(), EMPTY, EMPTY, "Query object doesn't have `aggregate` field");
  }
  return std::make_tuple(common::Status::QueryParsingError(), EMPTY, EMPTY, "Can't parse `aggregate` field");
}
 ...
 */
static std::tuple<common::Status, GroupAggregate, ErrorMsg> parse_group_aggregate_stmt(boost::property_tree::ptree const& ptree,
                                                                                       const std::string& field_name="group-aggregate") {
  bool components[] = {
    false, false, false
  };
  GroupAggregate result;
  std::stringstream error_fmt;
  auto aggregate = ptree.get_child_optional(field_name);
  if (aggregate) {
    // select query
    for (auto kv: *aggregate) {
      ...
      if (tag_name == "step") {
        if (components[0]) {
          // Duplicate "step" tag
          LOG(ERROR) << "Duplicate `step` tag in `" + field_name + "` statement";
          error_fmt << "duplicate `step` tag in `" << field_name << "` statement";
          break;
        } else {
          if (!value) {
            LOG(ERROR) << "Tag `step` is not set in `" + field_name + "` statement";
            error_fmt << "tag `step` is not set in `" << field_name << "` statement";
            break;
          }
          try {
            Duration step = DateTimeUtil::parse_duration(value.get().data(), value.get().size());
            result.step = step;
            components[0] = true;
          } catch (const BadDateTimeFormat& e) {
            LOG(ERROR) << "Can't parse time-duration: " + *value;
        ...
        if (!value) {
          LOG(ERROR) << "Tag `metric` is not set in `" + field_name + "` statement";
          error_fmt << "tag `metric` is not set in `" << field_name << "` statement";
          break;
        }
        if (components[1]) {
          // Duplicate "metric" tag
          LOG(ERROR) << "Duplicate `metric` tag in `" + field_name + "` statement";
          error_fmt << "duplicate `metric` tag in `" << field_name << "` statement";
          break;
        } else {
          for (auto child: aggregate->get_child("metric")) {
            ...
            if (metric) {
              if (!metric->empty() && metric->front() != '!') {
                ...
                LOG(ERROR) << "Metric name can't be empty or start with '!' symbol";
        ...
        if (components[2]) {
          // Duplicate "func" tag
          LOG(ERROR) << "Duplicate `func` tag in `" + field_name + "` statement";
          error_fmt << "duplicate `func` tag in `" << field_name << "` statement";
          break;
        } else {
          int n = 0;
          bool error = false;
          for (auto child: aggregate->get_child("func")) {
            ...
            if (fn) {
              ...
              if (status == common::Status::Ok()) {
                ...
                LOG(ERROR) << "Invalid aggregation function `" + fn.get() + "`";
          ...
          if (result.func.empty() && !error && value) {
            ...
            if (status == common::Status::Ok()) {
              ...
              LOG(ERROR) << "Aggregation function can't be found in the query";
  ...
  if (complete) {
    ...
    LOG(ERROR) << "Can't validate `" + field_name + "` statement, `step` field required";
    fullerr << "Can't validate `" << field_name << "` statement, `step` field required, " << error_fmt.str();
  } else if (components[1] == false) {
    LOG(ERROR) << "Can't validate `" + field_name + "` statement, `metric` field required";
    fullerr << "Can't validate `" << field_name << "` statement, `metric` field required, " << error_fmt.str();
  } else if (components[2] == false) {
    LOG(ERROR) << "Can't validate `" + field_name + "` statement, `func` field required";
    fullerr << "Can't validate `" << field_name << "` statement, `func` field required, " << error_fmt.str();
  }
  return std::make_tuple(common::Status::QueryParsingError(), result, fullerr.str());
}

/** Parse `oreder-by` statement, format:
 * { "oreder-by": "series", ... }
 */
static std::tuple<common::Status, OrderBy, ErrorMsg> parse_orderby(boost::property_tree::ptree const& ptree) {
  ...
  if (orderby) {
    ...
    if (stringval == "time") {
      ...
      return std::make_tuple(common::Status::Ok(), OrderBy::SERIES, ErrorMsg());
    } else {
      LOG(ERROR) << "Invalid 'order-by' statement";
      return std::make_tuple(common::Status::QueryParsingError(),
                             OrderBy::TIME,
...
parse_groupby(boost::property_tree::ptree const& ptree) {
  ...
  return std::make_tuple(common::Status::Ok(), tags, op, ErrorMsg());
}
...
static std::tuple<common::Status, Timestamp, Timestamp, ErrorMsg> parse_range_timestamp(boost::property_tree::ptree const& ptree,
    ...
    for(auto child: *range) {
      if (child.first == "from") {
        ...
        try {
          ...
          LOG(ERROR) << std::string("Can't parse begin timestmp, ") + e.what();
        ...
        try {
          ...
          LOG(ERROR) << std::string("Can't parse end timestmp, ") + e.what();
          if (!begin_set) {
            fmt << ", ";
          }
  ...
  if (!begin_set || !end_set) {
    if (error) {
      return std::make_tuple(common::Status::QueryParsingError(), begin, end, "Range field error: " + fmt.str());
    } else {
      return std::make_tuple(common::Status::QueryParsingError(), begin, end, "Range field is not set");
    }
  }
  return std::make_tuple(common::Status::Ok(), begin, end, ErrorMsg());
}
 ...
 */
static std::tuple<common::Status, std::vector<ParamId>, ErrorMsg> parse_where_clause(boost::property_tree::ptree const& ptree,
                                                                                     std::vector<std::string> metrics,
                                                                                     SeriesMatcher const& matcher) {
  common::Status status = common::Status::Ok();
  std::vector<ParamId> output;
  auto where = ptree.get_child_optional("where");
  if (where) {
    if (metrics.empty()) {
      LOG(ERROR) << "Metric is not set";
      return std::make_tuple(common::Status::QueryParsingError(), output, "Invalid where clause, metric is not set");
    }
 ...
 */
static std::tuple<common::Status, std::vector<Filter>, FilterCombinationRule, ErrorMsg>
        ...
        for (size_t i = 0; i < nitems; i++) {
          ...
          if (item) {
            ...
            try {
              ...
              LOG(ERROR) << metrics[ix] + " has bad filter value, can't parse floating point";
              error_fmt << "Query object filter field: " << metrics[ix]
                  << " has bad value, can't parse floating point";
              found_at_least_one = false;
              nfound = 0;
              status = common::Status::BadArg();
              break;
            }
        ...
        for (auto sub: *child) {
          if (sub.first == "require") {
            ...
            if (value == "all") {
              ...
              LOG(ERROR) << std::string("Unknown filter combiner ") + value;
              error_fmt << "Query object filter field has unknown filter combiner " << value;
              return std::make_tuple(common::Status::QueryParsingError(), result, rule, error_fmt.str());
            }
          } else {
            LOG(ERROR) << std::string("Unknown filter meta key ") + sub.first;
      ...
      for (size_t i = 0; i < nitems; i++) {
        ...
        if (item) {
          ...
          try {
            ...
            LOG(ERROR) << metrics[0] + " has bad filter value, can't parse floating point";
...
// QueryParser class //
// ///////////////// //
std::tuple<common::Status, boost::property_tree::ptree, ErrorMsg> QueryParser::parse_json(const char* query) {
  ...
  try {
    ...
    LOG(ERROR) << error.str();
    return std::make_tuple(common::Status::QueryParsingError(), ptree, error.str());
  }
  return std::make_tuple(common::Status::Ok(), ptree, ErrorMsg());
}

std::tuple<common::Status, QueryKind, ErrorMsg> QueryParser::get_query_kind(boost::property_tree::ptree const& ptree) {
  common::Status status;
  for (const auto& item: ptree) {
    if (item.first == "select") {
      ...
      if (status != common::Status::Ok()) {
        ...
        // TODO: Depricated
        return std::make_tuple(common::Status::Ok(), QueryKind::SELECT_META, ErrorMsg());
      } else {
        return std::make_tuple(common::Status::Ok(), QueryKind::SELECT, ErrorMsg());
      }
    } else if (item.first == "aggregate") {
      return std::make_tuple(common::Status::Ok(), QueryKind::AGGREGATE, ErrorMsg());
    } else if (item.first == "join") {
      return std::make_tuple(common::Status::Ok(), QueryKind::JOIN, ErrorMsg());
    } else if (item.first == "group-aggregate") {
      return std::make_tuple(common::Status::Ok(), QueryKind::GROUP_AGGREGATE, ErrorMsg());
    } else if (item.first == "group-aggregate-join") {
      return std::make_tuple(common::Status::Ok(), QueryKind::GROUP_AGGREGATE_JOIN, ErrorMsg());
    } else if (item.first == "select-events") {
      return std::make_tuple(common::Status::Ok(), QueryKind::SELECT_EVENTS, ErrorMsg());
    }
  }
  ...
  return std::make_tuple(common::Status::QueryParsingError(), QueryKind::SELECT, error_message);
}

std::tuple<common::Status, ErrorMsg> validate_query(boost::property_tree::ptree const& ptree) {
 ...
 * { "select": "meta:*", ...}
 */
std::tuple<common::Status, std::vector<ParamId>, ErrorMsg> QueryParser::parse_select_meta_query(
  ...
  if (!is_meta_query(name)) {
    return std::make_tuple(common::Status::QueryParsingError(), ids,
  ...
  return std::make_tuple(common::Status::Ok(), ids, ErrorMsg());
}
...
std::tuple<common::Status, std::vector<ParamId>, ErrorMsg>
...
static std::tuple<common::Status, ErrorMsg> validate_suggest_query(boost::property_tree::ptree const& ptree) {
  static const std::set<std::string> ALLOWED_STMTS = {
    "select",
    "metric",
    "tag",
  ...
  for (const auto& item: ptree) {
    ...
    if (ALLOWED_STMTS.count(keyword) == 0) {
      LOG(ERROR) << "Unexpected `" + keyword + "` statement";
      return std::make_tuple(common::Status::QueryParsingError(), "Unexpected field `" + keyword + "` in query object");
    }
  }
  return std::make_tuple(common::Status::Ok(), ErrorMsg());
}

static std::tuple<SuggestQueryKind, common::Status, ErrorMsg> get_suggest_query_type(boost::property_tree::ptree const& ptree) {
  auto child = ptree.get_child_optional("select");
  if (!child) {
    LOG(ERROR) << "Query object missing `select` field";
    return std::make_tuple(SuggestQueryKind::SUGGEST_ERROR,
                           common::Status::QueryParsingError(),
                           "Query object missing `select` field");
  }
  auto value = child->get_value_optional<std::string>();
  if (!value) {
    LOG(ERROR) << "Query object has invalid `select` field, single value expected";
    return std::make_tuple(SuggestQueryKind::SUGGEST_ERROR,
                           common::Status::QueryParsingError(),
  ...
  if (strval == "metric-names") {
    return std::make_tuple(SuggestQueryKind::SUGGEST_METRIC_NAMES, common::Status::Ok(), ErrorMsg());
  } else if (strval == "tag-names") {
    return std::make_tuple(SuggestQueryKind::SUGGEST_TAG_NAMES, common::Status::Ok(), ErrorMsg());
  } else if (strval == "tag-values") {
    return std::make_tuple(SuggestQueryKind::SUGGEST_TAG_VALUES, common::Status::Ok(), ErrorMsg());
  }
  LOG(ERROR) << "Query object has invalid `select` field, unknown target " + strval;
  return std::make_tuple(SuggestQueryKind::SUGGEST_ERROR,
                         common::Status::QueryParsingError(),
...
static std::tuple<common::Status, std::string> get_property(std::string name, boost::property_tree::ptree const& ptree) {
  auto child = ptree.get_child_optional(name);
  if (!child) {
    return std::make_tuple(common::Status::NotFound(), std::string());
  }
  auto value = child->get_value_optional<std::string>();
  if (!value) {
    return std::make_tuple(common::Status::BadArg(), std::string());
  }
  auto strval = value.get();
  return std::make_tuple(common::Status::Ok(), strval);
}

std::tuple<common::Status, std::shared_ptr<PlainSeriesMatcher>, std::vector<ParamId>, ErrorMsg>
QueryParser::parse_suggest_query(boost::property_tree::ptree const& ptree, SeriesMatcher const& matcher) {
  ...
  switch (kind) {
    ...
    case SuggestQueryKind::SUGGEST_TAG_NAMES:
      ...
      if (status != common::Status::Ok()) {
        LOG(ERROR) << "Metric name expected";
        return std::make_tuple(common::Status::QueryParsingError(), substitute, ids, "Metric name expected");
      }
    ...
    case SuggestQueryKind::SUGGEST_TAG_VALUES:
      ...
      if (status != common::Status::Ok()) {
        LOG(ERROR) << "Tag name expected";
        return std::make_tuple(common::Status::QueryParsingError(), substitute, ids, "Tag name expected");
      }
      results = matcher.suggest_tag_values(metric_name, tag_name, starts_with);
      break;
    case SuggestQueryKind::SUGGEST_ERROR:
      return std::make_tuple(common::Status::QueryParsingError(), substitute, ids, error);
  };
  ...
  return std::make_tuple(common::Status::Ok(), substitute, ids, ErrorMsg());
}

std::tuple<common::Status, ReshapeRequest, ErrorMsg> QueryParser::parse_select_query(
    boost::property_tree::ptree const& ptree,
  ...
  if (groupbytag) {
    ...
    if (result.group_by.transient_map.empty()) {
      return std::make_tuple(common::Status::NoData(), result, "Group-by statement doesn't match any series");
    }
  }
...
std::tuple<common::Status, ReshapeRequest, ErrorMsg> QueryParser::parse_select_events_query(
    boost::property_tree::ptree const& ptree,
    const SeriesMatcher &matcher) {
  ReshapeRequest result = {};
 ...
 */
static std::tuple<common::Status, ErrorMsg>
init_matcher_in_aggregate(
  ...
  for (u32 ix = 0; ix < ids.size(); ix++) {
    ...
    if (mpos == std::string::npos) {
      LOG(ERROR) << "Matcher initialization failed. Invalid series name.";
      return std::make_tuple(common::Status::BadData(), "Invalid series name `" + name + "`");
    }
...
std::tuple<common::Status, ReshapeRequest, ErrorMsg> QueryParser::parse_aggregate_query(
    boost::property_tree::ptree const& ptree,
    SeriesMatcher const& matcher) {
  ReshapeRequest result = {};
  result.select.global_matcher = &matcher;
  ...
  auto orderby = ptree.get_child_optional("order-by");
  if (orderby) {
    LOG(INFO) << "Unexpected `order-by` statement found in `aggregate` query";
    return std::make_tuple(common::Status::QueryParsingError(),
                           result,
...
init_matcher_in_group_aggregate(
  ...
  std::vector<ParamId> ids = req->select.columns.at(0).ids;
  auto matcher = std::make_shared<PlainSeriesMatcher>();
  for (auto id: ids) {
    auto sname = global_matcher.id2str(id);
    std::string name(sname.first, sname.first + sname.second);
...
std::tuple<common::Status, ReshapeRequest, ErrorMsg> QueryParser::parse_group_aggregate_query(
    boost::property_tree::ptree const& ptree,
    SeriesMatcher const& matcher) {
  ReshapeRequest result = {};
  result.select.global_matcher = &matcher;
  ...
  if (gagg.func.empty()) {
    LOG(ERROR) << "Aggregation fuction is not set";
    return std::make_tuple(status, result, "Aggregation fuction is not set");
  }
  if (gagg.step == 0) {
    LOG(ERROR) << "Step can't be zero";
...
static std::tuple<common::Status, ErrorMsg> init_matcher_in_join_query(
    ReshapeRequest*                 req,
    SeriesMatcher const&            global_matcher,
    std::vector<std::string> const& metric_names) {
  if (req->select.columns.size() < 2) {
    LOG(ERROR) << "Can't initialize matcher. Query is not a `JOIN` query.";
    return std::make_tuple(common::Status::BadArg(), "Can't initialize matcher. Query is not a `JOIN` query.");
  }
  if (req->select.columns.size() != metric_names.size()) {
    LOG(ERROR) << "Can't initialize matcher. Invalid metric names.";
    return std::make_tuple(common::Status::BadArg(), "Can't initialize matcher. Invalid metric names.");
  }
  std::vector<ParamId> ids = req->select.columns.at(0).ids;
  auto matcher = std::make_shared<PlainSeriesMatcher>();
  for (auto id: ids) {
    ...
    if (!boost::algorithm::starts_with(name, metric_names.front())) {
      LOG(ERROR) << "Matcher initialization failed. Invalid metric names.";
      return std::make_tuple(common::Status::BadData(), "Matcher initialization failed. Invalid metric names.");
    }
...
std::tuple<common::Status, ReshapeRequest, ErrorMsg> QueryParser::parse_group_aggregate_join_query(
    boost::property_tree::ptree const& ptree,
    SeriesMatcher const& matcher) {
  /*
   * { "group-aggregate-join": {
  ...
  // Metric name
  GroupAggregate gagg;
  std::tie(status, gagg, error) = parse_group_aggregate_stmt(ptree, "group-aggregate-join");
  if (status != common::Status::Ok()) {
    return std::make_tuple(status, result, error);
  }
  if (gagg.func.empty()) {
    LOG(ERROR) << "Aggregation fuction is not set";
    return std::make_tuple(common::Status::QueryParsingError(), result, "Aggregation fuction is not set");
  }
  if (gagg.func.size() != 1) {
    LOG(ERROR) << "Only one aggregaton fuction should be set";
    return std::make_tuple(common::Status::QueryParsingError(), result, "Single aggregation fuction expected");
  }
  if (gagg.step == 0) {
    LOG(ERROR) << "Step can't be zero";
    return std::make_tuple(common::Status::QueryParsingError(), result, "Step can't be zero");
  }
  if (gagg.metric.size() <= 1) {
    LOG(ERROR) << "Join query expects at least two metrics";
    return std::make_tuple(common::Status::QueryParsingError(), result, "Join query expects at least two metrics");
  }
  ...
  if (!tags.empty()) {
    LOG(ERROR) << "Group-by-tag/pivot-by-tag is not supported";
    return std::make_tuple(common::Status::QueryParsingError(), result, "Group-by-tag/pivot-by-tag is not supported");
  }
  ...
  size_t ncolumns = gagg.metric.size();
  size_t nentries = ids.size() / ncolumns;
  if (ids.size() % ncolumns != 0) {
    LOG(FATAL) << "Invalid `where` statement processing results";
  }
  u32 idix = 0;
...
std::tuple<common::Status, ReshapeRequest, ErrorMsg> QueryParser::parse_join_query(
    boost::property_tree::ptree const&  ptree,
    SeriesMatcher const&                matcher) {
  ReshapeRequest result = {};
  result.select.global_matcher = &matcher;
...
struct TerminalNode : qp::Node {
  ...
  TerminalNode(InternalCursor* cur) : cursor(cur) { }
...
std::tuple<common::Status, std::shared_ptr<Node>, ErrorMsg>
make_sampler(boost::property_tree::ptree const& ptree,
  ...
  try {
    ...
    return std::make_tuple(common::Status::Ok(), qp::create_node(name, ptree, req, next), ErrorMsg());
  } catch (const boost::property_tree::ptree_error& e) {
    auto err = ErrorMsg("Query object has invalid `apply` field ") + e.what();
    LOG(ERROR) << err;
    return std::make_tuple(common::Status::QueryParsingError(), nullptr, err);
  } catch (const QueryParserError& e) {
    auto err = ErrorMsg("Query object has invalid `apply` field ") + e.what();
    LOG(ERROR) << err;
    return std::make_tuple(common::Status::QueryParsingError(), nullptr, err);
  } catch (...) {
    LOG(ERROR) << std::string("Unknown query parsing error: ") <<
...
std::tuple<common::Status, std::vector<std::shared_ptr<Node>>, ErrorMsg> QueryParser::parse_processing_topology(
    boost::property_tree::ptree const& ptree,
    InternalCursor* cursor,
    const ReshapeRequest& req) {
