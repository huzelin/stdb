...
LogSequencer::LogSequencer() : counter_{0} { }
...
static std::tuple<common::Status, size_t> _write_frame(AprFilePtr& file, u32 size, void* array) {
  size_t outsize = 0;
  iovec io[2] = {
    { &size, sizeof(u32) },
    { array, size },
  ...
  if (status != APR_SUCCESS) {
    ...
    return std::make_tuple(common::Status::ErrIO(), 0u);
  }
  return std::make_tuple(common::Status::Ok(), outsize);
}

static common::Status _flush_file(AprFilePtr& file) {
  apr_status_t status = apr_file_flush(file.get());
  if (status != APR_SUCCESS) {
    log_apr_error(status, "Can't flush file");
    return common::Status::ErrIO();
  }
  return common::Status::Ok();
}

static std::tuple<common::Status, size_t> _read_frame(AprFilePtr& file, u32 array_size, void* array) {
  u32 size;
  size_t bytes_read = 0;
  ...
  return std::make_tuple(common::Status::Ok(), bytes_read);
}
...
common::Status LZ4Volume::write(int i) {
...
std::tuple<common::Status, size_t> LZ4Volume::read(int i) {
  assert(is_read_only_);
  Frame& frame = frames_[i];
  // Read frame
  u32 frame_size;
  ...
  int out_bytes = LZ4_decompress_safe_continue(&decode_stream_,
                                               ...
                                               BLOCK_SIZE);
  if(out_bytes <= 0) {
    return std::make_tuple(common::Status::BadData(), 0);
  }
  return std::make_tuple(common::Status::Ok(), frame_size + sizeof(u32));
}
...
LZ4Volume::LZ4Volume(LogSequencer* sequencer, const char* file_name, size_t volume_size)
  : path_(file_name)
    ...
    , bitmap_(std::make_shared<roaring::Roaring64Map>())
    , is_read_only_(false)
    , bytes_to_read_(0)
    , elements_to_read_(0)
    , sequencer_(sequencer) {
  LOG(INFO) << std::string("Open LZ4 volume ") + file_name + " for logging";
  clear(0);
  clear(1);
...
LZ4Volume::LZ4Volume(const char* file_name)
    ...
    , elements_to_read_(0)
{
  LOG(INFO) << std::string("Open LZ4 volume ") + file_name + " for reading";
  clear(0);
  clear(1);
...
common::Status LZ4Volume::flush_current_frame(FrameType type) {
...
common::Status LZ4Volume::require_frame_type(FrameType type) {
  Frame& frame = frames_[pos_];
...
common::Status LZ4Volume::append(u64 id, u64 timestamp, double value) {
...
common::Status LZ4Volume::append_blob(FrameType type, u64 id, const char* payload, u32 len) {
...
common::Status LZ4Volume::append(u64 id, const char* sname, u32 len) {
  return append_blob(FrameType::SNAME_ENTRY, id, sname, len);
}

common::Status LZ4Volume::append(u64 id, const u64* recovery_array, u32 len) {
...
std::tuple<common::Status, u32> LZ4Volume::read_next(size_t buffer_size, u64* id, u64* ts, double* xs) {
  if (elements_to_read_ == 0) {
    if (bytes_to_read_ <= 0) {
      // Volume is finished
      return std::make_tuple(common::Status::Ok(), 0);
    }
    pos_ = (pos_ + 1) % 2;
    clear(pos_);
    size_t bytes_read;
  ...
  return std::make_tuple(common::Status::Ok(), static_cast<int>(nvalues));
}

std::tuple<common::Status, u32> LZ4Volume::read_next(size_t buffer_size, InputLogRow* rows) {
  if (elements_to_read_ == 0) {
    if (bytes_to_read_ <= 0) {
      // Volume is finished
      return std::make_tuple(common::Status::Ok(), 0);
...
std::tuple<common::Status, const LZ4Volume::Frame*> LZ4Volume::read_next_frame() {
  if (bytes_to_read_ <= 0) {
    // Volume is finished
    return std::make_tuple(common::Status::Ok(), nullptr);
  }
  pos_ = (pos_ + 1) % 2;
  clear(pos_);
  size_t bytes_read;
  ...
  return std::make_tuple(common::Status::Ok(), frame);
}
...
const roaring::Roaring64Map& LZ4Volume::get_index() const {
  return *bitmap_;
}

common::Status LZ4Volume::flush() {
  Frame& frame = frames_[pos_];
...
void InputLog::add_volume(std::string path) {
  if (boost::filesystem::exists(path)) {
    LOG(ERROR) << std::string("Path ") + path + " already exists";
  }
...
void InputLog::delete_files() {
  LOG(INFO) << "Delete all volumes";
  for (auto& it: volumes_) {
    LOG(INFO) << std::string("Delete ") + it->get_path();
    it->delete_file();
  }
}
...
void InputLog::detect_stale_ids(std::vector<u64>* stale_ids) {
  ...
  roaring::Roaring64Map sum = roaring::Roaring64Map::fastunion(remaining.size(), remaining.data());
...
common::Status InputLog::append(u64 id, u64 timestamp, double value, std::vector<u64>* stale_ids) {
  common::Status result = volumes_.front()->append(id, timestamp, value);
...
common::Status InputLog::append(u64 id, const char* sname, u32 len, std::vector<u64>* stale_ids) {
  common::Status result = volumes_.front()->append(id, sname, len);
  if (result.Code() == common::Status::kOverflow && volumes_.size() == max_volumes_) {
    detect_stale_ids(stale_ids);
  }
  return result;
...
common::Status InputLog::append(u64 id, const u64 *rescue_points, u32 len, std::vector<u64>* stale_ids) {
  common::Status result = volumes_.front()->append(id, rescue_points, len);
  if (result.Code() == common::Status::kOverflow && volumes_.size() == max_volumes_) {
    detect_stale_ids(stale_ids);
  }
  return result;
...
std::tuple<common::Status, u32> InputLog::read_next(size_t buffer_size, u64* id, u64* ts, double* xs) {
  while (true) {
    if (volumes_.empty()) {
      return std::make_tuple(common::Status::Ok(), 0);
    }
...
std::tuple<common::Status, u32> InputLog::read_next(size_t buffer_size, InputLogRow* rows) {
  while (true) {
    if (volumes_.empty()) {
      return std::make_tuple(common::Status::Ok(), 0);
    }
...
std::tuple<common::Status, const LZ4Volume::Frame*> InputLog::read_next_frame() {
  while (true) {
    if (volumes_.empty()) {
      return std::make_tuple(common::Status::NoData(), nullptr);
    }
    if (volumes_.front()->is_opened() == false) {
      volumes_.front()->open_ro();
    }
    ...
    return std::make_pair(common::Status::Ok(), result);
  }
}
...
common::Status InputLog::flush(std::vector<u64>* stale_ids) {
  if (volumes_.empty()) {
    return common::Status::Ok();
  }
...
std::tuple<common::Status, int> get_concurrency_level(const char* root_dir) {
  // file name example: inputlog28_0.ils
  if (!boost::filesystem::exists(root_dir)) {
    return std::make_tuple(common::Status::NotFound(), 0);
  }
  ...
  return std::make_tuple(common::Status::Ok(), static_cast<int>(max_stream_id + 1));  // stream ids are 0 based indexes
}
    ...
    , svol_(0)
{
  if (concurrency_ == 0) {
    common::Status status;
    LOG(INFO) << "Trying to retreive previous concurrency level";
    int newlevel;
    std::tie(status, newlevel) = get_concurrency_level(rootdir);
    if (!status.IsOk()) {
      LOG(FATAL) << "Can't retreive concurrency level of the input log: " << status.ToString();
    } else {
      LOG(ERROR) << "Concurrency level of the input log is " << newlevel;
...
InputLog& ShardedInputLog::get_shard(int i) {
  if (read_only_) {
    LOG(FATAL) << "Can't write read-only input log";
  }
...
void ShardedInputLog::init_read_buffers() {
  if (!read_only_) {
    LOG(FATAL) << "Can't read write-only input log";
  }
...
int ShardedInputLog::choose_next() {
  ...
  for (;ixstart < read_queue_.size(); ixstart++) {
    if (read_queue_.at(ixstart).status.IsOk() &&
  ...
  for(size_t ix = ixstart + 1; ix < read_queue_.size(); ix++) {
    if (!read_queue_.at(ix).status.IsOk() ||
...
std::tuple<common::Status, u32> ShardedInputLog::read_next(
    size_t  buffer_size,
    u64*    idout,
    u64*    tsout,
    double* xsout)
{
  ...
  while(buffer_ix_ >= 0 && buffer_size > 0) {
    ...
    if (buffer.pos < buffer.frame->data_points.size) {
      ...
      const ParamId*   ids = buffer.frame->data_points.ids + buffer.pos;
      const Timestamp* tss = buffer.frame->data_points.tss + buffer.pos;
      ...
      if (buffer_ix_ < 0) {
        // No more data
        outstatus = common::Status::NoData();
        break;
      }
    }
  }
...
std::tuple<common::Status, u32> ShardedInputLog::read_next(size_t buffer_size, InputLogRow* rows) {
  buffer_size = std::min(static_cast<size_t>(std::numeric_limits<u32>::max()), buffer_size);
  if (!read_started_) {
    init_read_buffers();
  }
...
void ShardedInputLog::reopen() {
  if (!read_only_) {
    LOG(FATAL) << "Can't reopen write-only input log";
  }
  streams_.clear();
  for (int i = 0; i < concurrency_; i++) {
    std::unique_ptr<InputLog> log;
...
std::tuple<common::Status, int> ShardedInputLog::find_logs(const char* rootdir) {
  if (!boost::filesystem::exists(rootdir)) {
    return std::make_tuple(common::Status::NotFound(), -1);
  }
  ...
  return std::make_tuple(common::Status::Ok(), static_cast<int>(max_stream_id + 1));
}
