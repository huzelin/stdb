...
void test_retreiver() {
    ...
    EXPECT_EQ(status, AKU_EBAD_ARG);
    std::tie(status, act) = rt1.extract_ids(m);
    EXPECT_EQ(status, common::Status::Ok());
    EXPECT_EQ_COLLECTIONS(ids.begin(), ids.end(), act.begin(), act.end());

    SeriesRetreiver rt2({"bbb"});
    std::tie(status, act) = rt2.extract_ids(m);
    EXPECT_EQ(status, common::Status::Ok());
    EXPECT_EQ_COLLECTIONS(ids.begin() + 5, ids.end(), act.begin(), act.end());
    ...
    EXPECT_EQ(status, common::Status::Ok());
    EXPECT_EQ_COLLECTIONS(ids.begin() + 7, ids.begin() + 9, act.begin(), act.end());
    ...
    EXPECT_EQ(status, common::Status::Ok());
    EXPECT_EQ_COLLECTIONS(ids.begin() + 9, ids.begin() + 11, act.begin(), act.end());
}
...
BOOST_AUTO_TEST_CASE(Test_series_add_2) {
    ...
    aku_ParamId ids[10];
    auto nids = session->get_series_ids(sname, end, ids, 10);
    EXPECT_EQ(-1*nids, AKU_EBAD_DATA);
}
...
BOOST_AUTO_TEST_CASE(Test_group_aggregate_join_query_0) {
...

    {
        ...
        session->query(&cursor, query);
        EXPECT_TRUE(cursor.done);
        EXPECT_EQ(cursor.error, AKU_EQUERY_PARSING_ERROR);
    }
