...
static storage::InputLog* get_input_log(storage::ShardedInputLog* log) {
...
StorageSession::~StorageSession() {
  LOG(INFO) << "StorageSession is being closed";
  if (ilog_) {
    std::vector<u64> staleids;
    auto res = ilog_->flush(&staleids);
    if (res == common::Status::Overflow()) {
...
common::Status StorageSession::write(Sample const& sample) {
  ...
  switch (status) {
    ...
    case NBTreeAppendResult::FAIL_BAD_ID:
      LOG(ERROR) << "Invalid session cache, id = " + std::to_string(sample.paramid);
...
common::Status StorageSession::init_series_id(const char* begin, const char* end, Sample *sample) {
  ...
  common::Status status = SeriesParser::to_canonical_form(begin, end, ob, oe, &ksbegin, &ksend);
...
int StorageSession::get_series_ids(const char* begin, const char* end, ParamId* ids, size_t ids_size) {
  // Series name normalization procedure. Most likeley a bottleneck but
  // can be easily parallelized.
  const char* ksbegin = nullptr;
  const char* ksend = nullptr;
  ...
  if (nmetric == 1) {
    ...
    char series[LIMITS_MAX_SNAME];
    // Copy tags without metrics to the end of the `series` array
    int tagline_len = static_cast<int>(ksend - ksbegin + 1);  // +1 for space, cast is safe because ksend-ksbegin < LIMITS_MAX_SNAME
    const char* metric_end = ksbegin - 1;  // -1 for space
    char* tagline = series + LIMITS_MAX_SNAME - tagline_len;
    // TODO: remove
    if (tagline_len < 0) {
      LOG(FATAL) << "Invalid tagline length";
    }
    // end
    memcpy(tagline, metric_end, static_cast<size_t>(tagline_len));
    char* send = series + LIMITS_MAX_SNAME;
    ...
    for (int i = 0; i < nmetric; i++) {
      ...
      // TODO: remove
      if (metric_len < 0) {
        LOG(FATAL) << "Invalid metric length";
      }
      // end
...
int StorageSession::get_series_name(ParamId id, char* buffer, size_t buffer_size) {
  StringT name;
...
Storage::Storage(const char* path, const FineTuneParams &params)
  : done_{0}
  , close_barrier_(2) {
  ...
  if (bstore_type == "FixedSizeFileStorage") {
    LOG(INFO) << "Open as fxied size storage";
  ...
  if (status != common::Status::Ok()) {
    LOG(FATAL) << "Can't read series names";
  }
  // Update column store
...
void Storage::run_recovery(const FineTuneParams &params,
                           std::unordered_map<ParamId, std::vector<storage::LogicAddr>>* mapping) {
...
void Storage::initialize_input_log(const FineTuneParams &params) {
  if (params.input_log_path) {
    LOG(INFO) << std::string("WAL enabled, path: ") +
...
void Storage::run_inputlog_metadata_recovery(
  ...
  struct Visitor : boost::static_visitor<bool> {
    ...
    bool operator () (const storage::InputLogSeriesName& sname) {
      ...
      if (id == 0) {
        ...
        if (prev.second != 0) {
          ...
          LOG(ERROR) << "Series id conflict. Id " + std::to_string(id) + " is already taken by "
      ...
      if (create_new) {
        ...
        LOG(INFO) << "WAL-recovery create new column based on series name " + sname.value;
    ...
    bool operator () (const storage::InputLogRecoveryInfo& rinfo) {
      ...
      if (!strref.second) {
        // Id is not taken
        LOG(ERROR) << "Series id is not taken. Id " + std::to_string(curr_id) + ", recovery record will be skipped.";
        return true;
      }
...
static std::string to_isostring(Timestamp ts) {
  char buffer[0x100];
  int len = DateTimeUtil::to_iso_string(ts, buffer, 0x100);
  if (len < 1) {
    LOG(FATAL) << "Can't convert timestamp to ISO string";
  }
...
void dump_tree(std::ostream &stream,
               ...
               ParamId id,
               std::vector<storage::LogicAddr> rescue_points) {
...
common::Status Storage::generate_report(const char* path, const char *output) {
  ...
  if (status != common::Status::Ok()) {
    LOG(ERROR) << "Can't read rescue points";
    return status;
  }
  ...
  for(auto kv: mapping) {
    ParamId id = kv.first;
    std::vector<storage::LogicAddr> rescue_points = kv.second;
...
common::Status Storage::generate_recovery_report(const char* path, const char *output) {
  auto metadata = std::make_shared<MetadataStorage>(path);
  ...
  auto bstore = storage::FixedSizeFileStorage::open(metadata);
  auto cstore = std::make_shared<storage::ColumnStore>(bstore);
  ...
  for(auto kv: columns) {
    ParamId id = kv.first;
    std::shared_ptr<storage::NBTreeExtentsList> column = kv.second;
...
void Storage::close() {
  ...
  // TODO: remove
  LOG(INFO) << "Index memory usage: " + std::to_string(global_matcher_.memory_use());
  // END
  done_.store(1);
  metadata_->force_sync();
  close_barrier_.wait();
...
void Storage::close_specific_columns(const std::vector<u64>& ids) {
  LOG(INFO) << "Going to close " + std::to_string(ids.size()) + " ids";
  auto mapping = cstore_->close(ids);
  LOG(INFO) << std::to_string(ids.size()) + " ids were closed";
  if (!mapping.empty()) {
    for (auto kv: mapping) {
      u64 id;
      std::vector<u64> vals;
...
void Storage::_update_rescue_points(ParamId id, std::vector<storage::LogicAddr>&& rpoints) {
  metadata_->add_rescue_point(id, std::move(rpoints));
}

std::shared_ptr<StorageSession> Storage::create_write_session() {
  std::shared_ptr<storage::CStoreSession> session =
      std::make_shared<storage::CStoreSession>(cstore_);
  return std::make_shared<StorageSession>(shared_from_this(),
                                          session,
                                          inputlog_.get());
}

std::tuple<common::Status, bool> Storage::init_series_id(const char* begin, const char* end, Sample *sample, PlainSeriesMatcher *local_matcher) {
  u64 id = 0;
  bool create_new = false;
  {
    std::lock_guard<std::mutex> guard(lock_);
  ...
  return std::make_tuple(common::Status::Ok(), create_new);
}

int Storage::get_series_name(ParamId id, char* buffer, size_t buffer_size, PlainSeriesMatcher *local_matcher) {
...
std::tuple<common::Status, std::string>
...
common::Status Storage::new_database(const char     *base_file_name,
  ...
  if (volume_size > MAX_SIZE) {
    LOG(ERROR) << "Volume size is too big: " + std::to_string(volume_size) + ", it can't be greater than 15TB";
    return common::Status::BadArg();
  } else if (volume_size < MIN_SIZE) {
    LOG(ERROR) << "Volume size is too small: " + std::to_string(volume_size) + ", it can't be less than 1MB";
    return common::Status::BadArg();
  }
  ...
  if (!boost::filesystem::exists(volpath)) {
    LOG(INFO) << std::string(volumes_path) + " doesn't exists, trying to create directory";
    boost::filesystem::create_directories(volpath);
  } else {
    if (!boost::filesystem::is_directory(volpath)) {
      LOG(ERROR) << std::string(volumes_path) + " is not a directory";
      return common::Status::BadArg();
    }
  }
  ...
  if (!boost::filesystem::exists(metpath)) {
    ...
    if (!boost::filesystem::is_directory(metpath)) {
      LOG(ERROR) << std::string(metadata_path) + " is not a directory";
      return common::Status::BadArg();
    }
  }
...
common::Status Storage::remove_storage(const char* file_name, const char* wal_path, bool force) {
  ...
  if (!force) {
    ...
    auto fstore = storage::FixedSizeFileStorage::open(meta);
  ...
  auto comb_status = [](common::Status lhs, common::Status rhs) {
    return lhs == common::Status::Ok() ? rhs : lhs;
  };
  auto status = std::accumulate(statuses.begin(), statuses.end(), common::Status::Ok(), comb_status);
  if (status != common::Status::Ok()) {
    return common::Status::EAccess();
  }
  ...
  auto delete_file = [](std::string const& fname) {
    if (!boost::filesystem::remove(fname)) {
      LOG(ERROR) << fname + " file is not deleted!";
    } else {
      LOG(INFO) << fname + " was deleted.";
    }
  };
